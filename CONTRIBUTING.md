# Developer Guide for PowerWaterModels

In this guide, we aim to communicate the various code standards expected for PowerWaterModels.
...
These conventions should include any conventions applied across the InfrastructureModels ecosystem, specific to power and water systems (i.e, conventions from InfrastructureModels, PowerModels, PowerModelsDistribution, and WaterModels), with some additions specific to PowerWaterModels.

### Functions

Function additions should meet the following criteria:

- All functions should be clearly named, without abbreviations, and with underscores between words, e.g., `parse_files` or `constraint_pump_load`. In Python this is known as [`lower_case_with_underscores`](https://legacy.python.org/dev/peps/pep-0008/#descriptive-naming-styles). The exception to the abbreviation rule is cases where abbreviations would be expected in the modeling of power or water systems.
- All functions that are not prepended by an underscore `_` will be exported by default (i.e., when a user uses `using PowerWaterModels`). Public functions should have a detailed docstring instructing on usage.
...
PowerWaterModels follows the Semantic Versioning ([SemVer](https://semver.org/)) convention of `Major.minor.patch`, where `Major` indicates breaking changes, `minor` indicates non-breaking feature additions, and `patch` indicates non-breaking bugfixes.

Currently, because `Major==0`, `minor` indicates breaking changes and `patch` indicates any non-breaking change, including both feature additions and bugfixes. Once PowerWaterModels reaches `v1.0.0`, we will adhere strictly to the SemVer convention.
...
These branches will contain only breaking changes and will not be merged into master until a new version is ready to be tagged.
